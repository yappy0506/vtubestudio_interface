...
- 実装は `vtsd/` パッケージに配置し、`python -m vtsd` で起動する。
- WebSocket クライアントは Python 標準ライブラリに実装がないため、最小依存として `websockets` を使用する。
- Named Pipe は `ctypes` で Windows API（`CreateNamedPipe/ConnectNamedPipe/ReadFile/WriteFile`）を呼び出す。
- `auth_state` のゲートは通常APIに適用し、`APIStateRequest` / `AuthenticationTokenRequest` / `AuthenticationRequest` は例外として未認証時も `call` 可能にしている（仕様 7.3 の「認証そのものの操作は例外」を拡張解釈）。
